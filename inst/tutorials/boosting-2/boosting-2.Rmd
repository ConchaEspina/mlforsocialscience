...
```{r}
# data(drugs)
drugs <- read.csv("Drug_Consumption_Quantified.csv")
```
...
```{r}
set.seed(8303)
xgb <- train(D_LSD ~ Age + Gender + Education + Nscore + Escore +
...
```{r}
set.seed(8303)
mboost <- train(D_LSD ~ Age + Gender + Education + Nscore + Escore +
                Oscore + Ascore + Cscore + Impulsive + SS,
                data = drugs_train,
                method = "glmboost",
                trControl = ctrl,
...
```{r}
set.seed(8303)
ranger <- train(D_LSD ~ Age + Gender + Education + Nscore + Escore + 
                Oscore + Ascore + Cscore + Impulsive + SS,
                data = drugs_train,
                method = "ranger",
                trControl = ctrl,
...
```{r}
d_matrix <- model.matrix(D_LSD ~ Age + Gender + Education + Nscore + Escore + 
                           Oscore + Ascore + Cscore + Impulsive + SS, data = drugs)[,-1]
```
